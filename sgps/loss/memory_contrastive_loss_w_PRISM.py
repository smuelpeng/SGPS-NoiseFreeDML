...
class MemoryContrastiveLossPRISM(nn.Module):
    ...
    def forward(self, inputs_col, targets_col, inputs_row, target_row,is_noise=None, update_center=False):

        n = inputs_col.size(0)
        if inputs_row.shape[0]==0:
            return torch.Tensor([0]).cuda(), torch.ones_like(targets_col,dtype=bool).cuda()

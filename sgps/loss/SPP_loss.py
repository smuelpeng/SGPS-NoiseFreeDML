...
@LOSS_REGISTRY.register("RSPP_loss")
class RSPPLoss(nn.Module):
    ...
    def forward(self,
        ...
        # feat_query = feat_q[:, 0]
        # feat_main = feat_q[:, 1:self.IA_NUM_MAIN+1]
        # feat_main = feat_k[:, 1:self.IA_NUM_MAIN+1]
        # feat_sub = feat_q[:, self.IA_NUM_MAIN+1:]
        # feat_sub = feat_k[:,self.IA_NUM_MAIN+1:]
        feat_query = torch.index_select(
            feat_q, 1, torch.Tensor([0]).type(torch.int32).cuda()).squeeze(1)
        feat_main = torch.index_select(
            feat_q, 1, torch.arange(1, self.IA_NUM_MAIN + 1).cuda())
        feat_sub = torch.index_select(
            feat_q, 1, torch.arange(self.IA_NUM_MAIN+1, group_size).cuda())
        attention_mask = self.attention_module(feat_sub)

        query_label = targets[:, 0:1]
        main_label = targets[:, 1:(self.IA_NUM_MAIN+1)]
        sub_label = targets[:, (self.IA_NUM_MAIN+1):]
        # calculate filter q
        keep_bool, keep_prototype_weight, prototype_label = self.get_PRISM_filter(
            feat_query, query_label)          
        maks_neg_main = self.cal_neg_mask(query_label, main_label)
        ...
        bank_loss = (loss_bank_neg + loss_bank_pos) / self.batch_size
        batch_loss = self.cls_weight * loss_main + \
            self.con_weight * loss_switch  

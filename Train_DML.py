...
import argparse
import numpy as np
import dataloader_DML as dataloader
...
parser.add_argument('--id', default='DML')
parser.add_argument('--tau', default=5, type=float, help='filtering coefficient')
parser.add_argument('--data_path', default='./data/DML_org', type=str, help='path to dataset')
parser.add_argument('--seed', default=123)
parser.add_argument('--gpuid', default=0, type=int)
parser.add_argument('--pretrained', default=True, type=bool)
parser.add_argument('--num_class', default=14, type=int)
parser.add_argument('--num_batches', default=1000, type=int)
parser.add_argument('--dataset', default="DML", type=str)
...
parser.add_argument('--cfg', default="cfg.file", type=str)
...
# import wandb
# wandb.init(project="noisy-label-project-DML", entity="...")
...
def warmup(net,optimizer,dataloader):
    ...
    for batch_idx, (inputs, labels, path) in enumerate(dataloader):      
        ...
        # denan
        # if penalty.any().isnan():
        if torch.isnan(penalty):
            penalty = torch.zeros(1).cuda()
        L = loss + penalty       
        L.backward()  
        optimizer.step() 
...
def val(net,val_loader,k):
    ...
    if acc > best_acc[k-1]:
        ...
        torch.save(net.state_dict(), f'{save_point}.best')
...
def Calculate_JSD(epoch,model1, model2):
    ...
    for batch_idx, (inputs, targets, path) in enumerate(eval_loader):
        ...
        # print(out.shape, F.one_hot(targets, num_classes = args.num_class).shape)
        # print(targets)
        dist = JS_dist(out,  F.one_hot(targets, num_classes = args.num_class))
        # print(batch_idx, batch_size, dist.shape, len(eval_loader), num_samples)
...
cfg.MODEL.HEAD.NUM_CLASSES = args.num_class
log = open('./checkpoint/%s.txt'%args.id,'w')     
log.flush()

loader = dataloader.DML_dataloader(root=args.data_path, 
...
# if args.dataset in ['SOP','CUB','CARS']:
if True:
    model = get_model_res50()
    net1  = create_model_res50()
    net2  = create_model_res50()
    ...
    net1  = create_model()
    net2  = create_model()
    optimizer1 = optim.AdamW(net1.parameters(), lr=args.lr, weight_decay=5e-4)
    optimizer2 = optim.AdamW(net2.parameters(), lr=args.lr, weight_decay=5e-4)
...
## Optimizer and Learning Rate Scheduler 
# optimizer1 = optim.SGD(net1.parameters(), lr=args.lr, momentum=0.9, weight_decay=5e-5)
# optimizer2 = optim.SGD(net2.parameters(), lr=args.lr, momentum=0.9, weight_decay=5e-5)
...
model_name_1 = 'DML_net1.pth.tar'
model_name_2 = 'DML_net2.pth.tar'
...
for epoch in range(0, args.num_epochs+1):   
    val_loader = loader.run(args.dataset, 0, 'val')
    
    if epoch>100:
        nb_repeat =3  ## Change how many times we want to repeat on the same selection

    if epoch<warm_up:             
        train_loader = loader.run(args.dataset, 0,'warmup')
        print('Warmup Net1')
        ...
        num_samples = args.num_batches*args.batch_size
        eval_loader = loader.run(args.dataset, 0.5,'eval_train')
        ...
        for i in range(nb_repeat):
            print('\n\nTrain Net1')
            labeled_trainloader, unlabeled_trainloader = loader.run(args.dataset, SR, 'train', prob=prob2,  paths=paths2)         ## Uniform Selection
            train(epoch, net1, net2, optimizer1, labeled_trainloader, unlabeled_trainloader)                        ## Train Net1
            acc1 = val(net1,val_loader,1)

        print('\n==== Net 1 evaluate next epoch training data loss ====') 
        eval_loader   = loader.run(args.dataset, SR,'eval_train')
        ...
        for i in range(nb_repeat):
            print('\nTrain Net2')
            labeled_trainloader, unlabeled_trainloader = loader.run(args.dataset, SR, 'train', prob=prob1, paths=paths1)           ## Uniform Selection
    ...
    log.flush() 
    test_loader = loader.run(args.dataset, 0,'test')  
